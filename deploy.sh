...
# 1. Fix IPv6 routing hang on Linux VPS (common cause of npm hanging on spinner in Iran)
if [ "$(id -u)" -eq 0 ]; then
  echo "🔧 Optimizing network stack (disabling broken IPv6 routes)..."
...
# 5. Install dependencies without native compilation
echo "⚡ Installing packages (Pure JavaScript, 0 native compilation)..."
npm install

# 6. Build production bundle
echo "🔨 Building frontend and backend bundle..."
npm run build
