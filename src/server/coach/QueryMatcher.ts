...
export class QueryMatcher {
  ...
  match(rawQuery: string): ScenarioMatchCandidate[] {
    ...
    const coreIds = ['scen_1', 'scen_2', 'scen_3', 'scen_4', 'scen_5', 'scen_6', 'scen_7', 'scen_8', 'scen_intimacy_request_1'];
  ...
  private evaluateConceptSignals(scenarioId: string, query: string, tokens: string[]): number {
    switch (scenarioId) {
      ...
      case 'scen_3': {
        // ۳. شروع گفتگو و یخ‌شکنی حضوری در محیط‌های واقعی، آموزشی، اجتماعی و عمومی
        ...
        const hasOpenerAction = /سر صحبت|یخ شکنی|کلید بزنم|باب اشنایی|شروع صحبت|شروع مکالمه|اغاز کنم|جمله اول|نزدیک بشم|حرف بزنم|اشنا بشم|سخن|مکالمه|باب اشنایی بذارم|باب اشنایی باز کنم|صحبت رو شروع|حرف زدن رو شروع|برم جلو|برم سمت|شروع به صحبت/.test(query);
        const hasSocialSetting = /کافه|مهمانی|مهمونی|ورکشاپ|کارگاه|کتابخانه|کتابخونه|پرواز|فرودگاه|ایونت|سمینار|دانشگاه|همکلاسی|جمع غریبه|نمایشگاه کتاب|نمایشگاه|گالری|گالری عکاسی|سالن انتظار|اسانسور|سلف|حیاط دانشگاه|بغل دستیم|کسی که نمیشناسم|غریبه|رویداد|باشگاه|مترو|پارک|محیط کاری|همکار جدید|محل کار|میز کناری/.test(query);
        ...
        if (hasOpenerAction && /دختر|پسر|غریبه|کراش|هم کلاسی|تنها میشیم|کنار هم نشستیم|تنها نشسته بود|میز کناریه/.test(query)) return 7.0;
        return 0;
      }

      case 'scen_4': {
        // ۴. کنایه، کل‌کل، ادعا، تیکه، مسخره کردن، تست اعتمادبه‌نفس، پررویی و ادعا
        if (/^(پرو|پررو|پررویی|گستاخ)$/.test(query) || /\b(پررو|پرو|پررویی)\b/.test(query)) {
          return 8.5;
        }
        if (/جواب تیکه|کنایه زد|تحویل میگیری|پررو هستی|پرو هستی|جواب کل کل|مسخره کرد|طاقچه بالا|شوخی سنگین|بچه ای|بچگون|بچه بازی|بچه شدی|ادعات میشه|طعنه|خودشیفته|خاکی باش|ادعاها به قیافت|چقدر خودشیفته|اعتماد به نفست|فکر کردی خیلی زرنگی|لحن تمسخر|چقدر خوش خیالی/.test(query)) {
          return 8.5;
        }
        ...
        const hasTeaseLabel = /تیکه|کنایه|مسخره|تمسخر|کل کل|طاقچه بالا|پرو|پررو|تحویل|ادعا|بچه|بچگونه|بچه سال|قیافه|طعنه|خودشیفته|مغرور|خاکی باش|پروی|پرروی|دست انداختن|زرنگ|زرنگی|خوش خیال|خوش خیالی/.test(query);
        const hasTeaseActionContext = /زد|گفت|نوشت|انداخت|کرد|میگیری|میشه|هستی|شدی|چی بگم|جواب|لحنش|گف|درنیار|اطرافیان|فکر میکنی|فکر کردی|به قیافت|پیام داده که فکر کردی|لحن تمسخر/.test(query);
        if (hasTeaseLabel && hasTeaseActionContext && !/استوری/.test(query)) return 7.0;
        return 0;
      }
