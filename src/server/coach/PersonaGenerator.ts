...
export class PersonaGenerator {
  ...
  private static synthesizeVariations(
    ...
    // 0. High-Priority Specific Intent Categories (Conflict, Rejection, Sulking, Teasing, Salary, Anger)
    // Category: Anger / Nervousness / Outburst / Temper (عصبی، عصبانی، عصبانیت، خیلی عصبی، پرخاش...)
    ...
    // Category: Sulking / Coldness / Ghosting (قهر کرده، سرد شده، دلخور، سرسنگین...)
    ...
    // Category: Appearance Criticism (قیافت به دلم نمیشینه، زشتی، لاغری، چاقی...)
    ...
    // Category: Mockery in Public / Teasing / Roasting (مسخره‌م کرد، ضایع کردن، تیکه انداختن جلوی بقیه...)
    ...
    // Category: Salary / Income (حقوق، درآمد، چقدر درمیاری...)
    ...
    // Category: Quietness / Shy / Silence (کم‌حرفی، چرا ساکتی، چرا حرف نمی‌زنی...)
    ...
    // Category: Crisis / Hopelessness / Depression / Self-harm (بمیرم، خودکشی، خسته شدم از زندگی، بریدم، مرگ، پوچی...)
    if (/بمیرم|خودکشی|بکشم خودمو|از زندگی خسته|بریدم|امیدی ندارم|پوچی|ناامیدی|مرگ|تموم کنم/.test(combinedContext)) {
      return {
        charismatic: 'ارزش وجودی و گوهر درونی تو فراتر از بحران‌ها و تلاطم‌های گذراست. تاریک‌ترین ساعات شب همیشه درست قبل از طلوع سر می‌رسن؛ تو قوی‌تر از اونی هستی که این فصل سخت نقطه پایان داستانت باشه.',
        funny: 'دنیا هنوز پر از قهوه‌های داغ، فیلم‌های ندیده و شوخی‌های نشنیده‌ست که حیفه الان جا بزنی! بیا یکم با هم گپ بزنیم و آروم بشیم.',
        confident: 'این حجم از خستگی نشانه اینه که زیر فشار خیلی زیادی جنگیدی، نه اینکه ناتوان باشی. الان وقت تصمیم‌گیری نیست؛ یک نفس عمیق بکش، سرت رو بالا بگیر و اجازه بده این طوفان بگذره.',
        mysterious: 'انسان‌های عمیق همیشه از دل تاریک‌ترین اقیانوس‌ها عبور می‌کنن تا نور اصیل درونشون رو پیدا کنن؛ این درد پیله‌ایه که قراره ازت انسانی مستحکم بسازه.',
        mature: 'اولاً که دنیا بدون حضور تو جای خالی بزرگی داره، پس اصلاً به رفتن فکر نکن. از نظر روانی وقتی ذهن زیر فشار شدیده دچار دید تونلی میشه؛ این فقط خستگی بیوشیمیایی گذراست و می‌گذره.'
      };
    }

    // Category 8: Love / Confession / Affection (دوستت دارم، عاشقتم، بهت حس دارم، دلم برات تنگ شده...)
    if (/دوستت دارم|خیلی دوست دارم|عاشقتم|بهت حس دارم|ازت خوشم اومده|کراش دارم|دلم برات تنگ شده/.test(combinedContext)) {
      return {
    ...
    // Category 15: Distance / Relocation (راه دوره، شهر دیگه، فاصله جغرافیایی، مهاجرت...)
    ...
    // Category 16: Salary / Financial Privacy (حقوقت چقدره، چقدر درآمد داری، چقدر پول، وضعت چطوره...)
    if (/حقوق|درآمد|درامد|چقدر درمیاری|چقدر پول|حقوقت|وضع مالی|حساب بانکی/.test(combinedContext)) {
      return {
        charismatic: 'میزان درآمد یک عدد شخصیه، اما کیفیت نگاه و آرامش ذهنی تنها چیزیه که واقعاً ارزش داره.',
        funny: 'انقدری هست که از پس یک قهوه عالی بربیایم، ولی برای خریدن برج میلاد هنوز چند سالی وقت لازمه!',
    ...
    // Category 17: Mockery in Public / Teasing / Roasting (مسخره‌م کرد، ضایع کردن، تیکه انداختن جلوی بقیه...)
    ...
    // Category 18: Appearance Criticism (قیافت به دلم نمیشینه، زشتی، لاغری، چاقی...)
    if (/قیاف|قیافت|به دلم نمیشین|ظاهر|تیپت|زشت|لاغر|چاق|قد کوتا/.test(combinedContext)) {
      return {
        charismatic: 'سلیقه‌ها متفاوته و زیبایی در نگاه بیننده‌ست؛ کسی که کاریزمای واقعی رو بشناسه نیازی به توجیه ظاهر نداره.',
        funny: 'خوبه که زود گفتی، چون منم داشتم فکر می‌کردم چطور محترمانه بگم استانداردمون خیلی متفاوته!',
    ...
    // Category 19: Quietness / Shy / Silence (کم‌حرفی، چرا ساکتی، چرا حرف نمی‌زنی...)
    if (/کم\s*حرف|ساکت|چرا حرف نمیزنی|چرا کم حرفی|کم حرفم|حرفی نداری/.test(combinedContext)) {
      return {
        charismatic: 'من ترجیح میدم زمانی صحبت کنم که کلامم ارزش سکوت رو داشته باشه؛ شنونده دقیق بودن قدرت کاریزماتیک منه.',
        funny: 'دارم انرژی کلماتم رو ذخیره می‌کنم تا جاهایی که واقعاً ارزش شنیدن داره خرج کنم!',
    ...
    // Category 20: Sulking / Coldness / Ghosting (قهر کرده، سرد شده، دلخور، سرسنگین...)
    if (/قهر|سرد شده|دلخور|سرسنگین|ناراحت شده|چرا قهر کردی/.test(combinedContext)) {
      return {
        charismatic: 'مسائل با گفتگوی صمیمانه حل میشن نه با فاصله گرفتن؛ هر زمان آماده گفتگوی آرام بودی مشتاقم بشنومت.',
        funny: 'قهر کردن کالری زیادی می‌سوزونه! بیا به جای این کار، انرژیمون رو برای یک قهوه و صحبت واقعی بذاریم.',
    ...
    // 3. High-value persona synthesis from clean scenario retort (guaranteeing 5 COMPLETELY DISTINCT, independent sentences without repeating cleanCore)
    if (clean.length >= 4 && !clean.includes('علت تست') && !clean.includes('اشتباه بزرگ') && !clean.includes('مرحله قرار')) {
      ...
      const funnyPool = [
        'خوبه که حداقل خلاقیتت بالاست! ولی برای سنجش عیار من باید تمرین بیشتری بکنی 😉',
        'اگه با این فرمون جلو بری باید از دفعه بعد قبل چت با هم بیمه حوادث کلامی رد کنیم! 😄',
        'تازه این حالت پاور سیوینگمه؛ اگه دنده پنج برم که کلاً کنترل مکالمه از دستت در میره 😉',
        'عاشق این کل‌کل‌های رندانه‌تم! ولی خب رکورد حاضرجوابی خونسردانه فعلاً دست منه 😄'
      ];

      const confidentPool = [
        'اعتمادبه‌نفس و فریم من با چند کلمه بالا و پایین نمیره؛ موضع و ارزش‌هام کاملاً روشنه.',
        'جذابیت گفتگو به احترامه؛ من توی فضایی که پرستیژ دوطرفه حفظ نشه انرژی نمی‌ذارم.',
        'من پای حرفم محکم می‌ایستم و با آرامش کنترل فریم رو حفظ می‌کنم.',
        'من برای وقت و استانداردهام ارزش قائلم؛ مکالمه وقتی باکیفیت و دوطرفه باشه ارزشمنده.'
      ];

      const mysteriousPool = [
        'سکوت در برابر بعضی حرف‌ها رساترین پاسخه؛ لایه‌های عمیق‌تر رو به هرکسی نشون نمیدم.',
        'همیشه جالب‌ترین بخش آدما اون چیزیه که توی نگاه اول رو نمی‌کنن؛ صبور باش.',
        'کلماتی که پشت این آرامش پنهان شدن کشش بیشتری دارن؛ برای کشفشون باید عمیق‌تر نگاه کنی.',
        'بعضی مکالمه‌ها مثل کتاب‌های چندجلدی می‌مونن؛ ورق زدن فصل‌های بعدیش صبوری می‌خواد.'
      ];

      const maturePool = [
        'بلوغ رفتاری یعنی بدونی به چی بخندی و از کنار چی با لبخند بگذری؛ روز خوبی داشته باشی.',
        'درک متقابل و وقار کلامی همیشه کلید طلایی یک ارتباط پایدار و باکلاسه.',
        'احترام متقابل و آرامش نشانه یک ارتباط پخته و استواره؛ نیازی به تنش و جدل بی‌حاصل نیست.',
        'انرژی منفی جایی توی دایره معاشرت من نداره؛ کیفیت و عمق ارتباط برام همیشه در اولویته.'
      ];

      return {
        charismatic: cleanCore,
        funny: funnyPool[rIdx % funnyPool.length],
        confident: confidentPool[(rIdx + 1) % confidentPool.length],
        mysterious: mysteriousPool[(rIdx + 2) % mysteriousPool.length],
        mature: maturePool[(rIdx + 3) % maturePool.length]
      };
    }
