  ...
  const parseFiveStyles = (content: string, structuredData?: import('../types.js').AIScenarioQueryResponse) => {
    ...
    const parsedSections = styleDefs.map((def) => {
      ...
      if (!quote) {
        ...
        if (def.id === 'charismatic') {
          quote = 'مدیریت این موقعیت نیازمند رفتاری سنجیده، باوقار و با پرستیژ بالاست؛ بدون نیاز به تاییدخواهی، ارزشت رو در کلام و عمل حفظ کن.';
          tip = tip || 'لحن باوقار، مکث طلایی و زبان بدن مسلط.';
        } else if (def.id === 'funny') {
          quote = 'خوبه که حداقل رندی و شوخ‌طبعی رو چاشنی کردی! ولی برای به چالش کشیدن من هنوز باید تمرین بیشتری بکنی 😄';
          tip = tip || 'پوزخند خونسرد، حاضرجوابی بازیگوشانه و حفظ فریم ارتباط.';
          explanation = explanation || 'تحلیل مربی: شوخ‌طبعی رندانه تنش مکالمه را کاهش داده و جذابیت کلامی ایجاد می‌کند.';
        } else if (def.id === 'confident') {
          quote = 'من برای زمان، انرژی و چارچوب‌های خودم ارزش قائلم؛ وقتی مکالمه دوطرفه و شفاف باشه با کمال میل ادامه می‌دیم.';
          tip = tip || 'لحن محکم و خونسرد، صدای شمرده و بدون شتاب‌زدگی.';
          explanation = explanation || 'تحلیل مربی: قاطعیت توام با احترام، استانداردهای بالای فردی را تثبیت می‌کند.';
        } else if (def.id === 'mysterious') {
          quote = 'همیشه جذاب‌ترین بخش ماجرا همونیه که توی نگاه اول فاش نمیشه؛ بعضی ناگفته‌ها کشش خیلی بیشتری ایجاد می‌کنن ✨';
          tip = tip || 'نگاه نافذ، پاسخ چندلایه و برانگیختن کنجکاوی طرف مقابل.';
          explanation = explanation || 'تحلیل مربی: ابهام هدفمند اشتیاق مخاطب را برای کشف بیشتر حفظ می‌کند.';
        } else if (def.id === 'mature') {
          quote = 'دیدگاهت کاملاً قابل درکه؛ بلوغ ارتباطی یعنی بتونیم در اوج متانت و با احترام، تعامل رو به تعادل برسونیم.';
          tip = tip || 'آرامش درونی، همدلی عمیق و پرهیز از رفتارهای واکنشی.';
          explanation = explanation || 'تحلیل مربی: پاسخ متین و پخته فضا را برای درک متقابل آماده می‌سازد.';
        } else {
          quote = 'من برای زمان و چارچوب خودم ارزش قائلم و با آرامش و اعتمادبه‌نفس تعامل رو مدیریت می‌کنم.';
          tip = tip || 'لحن محکم و خونسرد، صدای شمرده و بدون شتاب‌زدگی.';
        }
      }
